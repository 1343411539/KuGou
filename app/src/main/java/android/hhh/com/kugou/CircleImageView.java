...
public class CircleImageView extends AppCompatImageView{
    ...
    private BitmapShader initBitmapShader(BitmapDrawable drawable) {
        Bitmap bitmap = drawable.getBitmap();//通过BitmapDrawable的方法,获取ImageView所展示图片的Bitmap
        BitmapShader bitmapShader = new BitmapShader(bitmap, Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);//通过此Bitmap创建一个着色器BitmapShader
        float scale = Math.max(width / bitmap.getWidth(), height / bitmap.getHeight());//根据图片Bitmap的宽高和控件CircleImageView的宽高，通过缩放矩阵Matrix对BitmapShader进行适当缩放，使图片撑满控件
